...
import { isAllowedExternalHref, resolvePreviewLinkTarget } from "../core/links";
...
import {
	chooseMarkdownSource,
	readDiskMarkdown,
} from "../core/markdownSourcePure";
import type {
	PreviewCheck,
	PreviewMode,
	PreviewState,
...
interface PreviewEntry {
	...
	surface: "dynamic" | "custom";
	ready: boolean;
	replaceAfterReady?: PreviewEntry;
...
export class MarkdownWorkbenchPanel
	implements vscode.Disposable, vscode.WebviewPanelSerializer
{
	...
	public async revealActive(modeOverride?: PreviewMode): Promise<boolean> {
		...
		if (editor?.document.languageId === "markdown") {
			await this.openOrRevealPreview(
				editor.document.uri,
				undefined,
				editor.document,
				{
		...
		const entry = this.activeSourceUri
			? this.getEntry(this.activeSourceUri)
			: undefined;
		if (!entry) {
			return false;
		}
	...
	public getActiveSourceUri(): vscode.Uri | undefined {
		return this.activeSourceUri;
	}

	public getSourceViewColumn(
		sourceUri: vscode.Uri,
	): vscode.ViewColumn | undefined {
		return this.getEntry(sourceUri)?.sourceViewColumn;
	}

	public isActiveInlinePreview(): boolean {
		const entry = this.activeSourceUri
			? this.getEntry(this.activeSourceUri)
			: undefined;
		return entry?.panel.active === true && entry.previewMode === "inline";
	}

	public async showSourceForActivePreview(): Promise<boolean> {
		const entry = this.activeSourceUri
			? this.getEntry(this.activeSourceUri)
			: undefined;
		if (!entry) {
		...
		await this.showSourceForEntry(entry);
		return true;
	}

	private async showSourceForEntry(entry: PreviewEntry): Promise<void> {
		await this.openSourceEditorForEntry(entry);
	}

	private async openSourceEditorForEntry(
		entry: PreviewEntry,
	): Promise<vscode.TextEditor> {
		if (entry.surface === "custom") {
			await vscode.commands.executeCommand("reopenActiveEditorWith", "default");
			const editor = vscode.window.visibleTextEditors.find(
				(candidate) =>
					candidate.document.uri.toString() === entry.sourceUri.toString(),
			);
			if (editor) {
				return editor;
			}
		...
		return vscode.window.showTextDocument(document, {
			viewColumn:
				entry.sourceViewColumn ??
				entry.panel.viewColumn ??
	...
	public attachCustomEditor(
		...
		entry.ready = false;
		entry.replaceAfterReady = existing;
		entry.initialFragment = initialFragment;
		this.activeSourceUri = sourceUri;
	}
	...
	public async updateAll(): Promise<void> {
		await Promise.all(
			Array.from(this.previews.values()).map((entry) =>
	...
	public async deserializeWebviewPanel(
		...
		const savedState = isSerializedPreviewState(state) ? state : undefined;
		if (!savedState) {
			panel.dispose();
			return;
		}
		...
		try {
			const document = await vscode.workspace.openTextDocument(
				vscode.Uri.parse(savedState.sourceUri),
			);
			const sourceColumn = savedState.sourceColumn as
				| vscode.ViewColumn
				| undefined;
			const entry = this.createEntry(
				document.uri,
				panel.viewColumn ?? resolveBesideColumn(sourceColumn),
				"beside",
				sourceColumn,
				panel,
				"dynamic",
			...
			await this.updateEntry(entry, document);
			if (typeof savedState.scrollLine === "number") {
				await panel.webview.postMessage({
					type: "scrollToLine",
					value: savedState.scrollLine,
				});
			}
		} catch {
			panel.dispose();
	...
	public isSyncingFromPreview(document?: vscode.TextDocument): boolean {
		if (document) {
			return (
				this.getEntry(document.uri)?.scrollSyncSuppressor.isActive("preview") ??
				false
			);
		}

		return Array.from(this.previews.values()).some((entry) =>
			entry.scrollSyncSuppressor.isActive("preview"),
		);
	}
	...
	private async openOrRevealPreview(
		...
		const document =
			knownDocument ?? (await vscode.workspace.openTextDocument(sourceUri));
		if (!isPreviewableMarkdown(document)) {
			return;
		}
		...
		const sourceViewColumn = this.resolveSourceViewColumn(
			document.uri,
			options.sourceViewColumn,
		);
		const targetColumn = this.resolvePreviewColumn(
			document.uri,
			previewMode,
			sourceViewColumn,
		);
		const existing = this.getEntry(document.uri);
		if (previewMode === "beside" && existing?.surface === "custom") {
			await this.openSidePreviewFromCustom(
				document,
				existing,
				sourceViewColumn,
			);
		...
		if (fragment) {
			await entry.panel.webview.postMessage({
				type: "scrollToAnchor",
				value: decodeFragment(fragment),
			});
		}
	}

	private async openSidePreviewFromCustom(
		document: vscode.TextDocument,
		existing: PreviewEntry,
		sourceViewColumn?: vscode.ViewColumn,
	): Promise<void> {
		const resolvedSourceColumn =
			sourceViewColumn ?? existing.sourceViewColumn ?? vscode.ViewColumn.One;
		await vscode.window.showTextDocument(document, {
			viewColumn: resolvedSourceColumn,
			preserveFocus: true,
			preview: false,
		});

		const targetColumn = resolveBesideColumn(resolvedSourceColumn);
		const entry = this.createEntry(
			document.uri,
			targetColumn,
			"beside",
		...
		this.activeSourceUri = document.uri;
		entry.panel.reveal(targetColumn, true);
		try {
			await this.updateEntry(entry, document);
		} catch (error) {
			this.previews.set(getPreviewKey(document.uri), existing);
			this.activeSourceUri = document.uri;
			entry.panel.dispose();
			throw error;
		}
	...
	private resolvePreviewColumn(
		...
		if (previewMode === "beside") {
			return resolveBesideColumn(sourceViewColumn);
		}

		return (
			this.resolveSourceViewColumn(sourceUri, sourceViewColumn) ??
	...
	private createEntry(
		...
		surface: "dynamic" | "custom" = "dynamic",
	): PreviewEntry {
		const panel =
			existingPanel ??
		...
		panel.onDidDispose(
			() => {
				clearPreviewScrollSyncTimer(entry);
				entry.scrollSyncSuppressor.dispose();
				if (this.getEntry(entry.sourceUri) === entry) {
					...
					if (this.activeSourceUri?.toString() === entry.sourceUri.toString()) {
						this.activeSourceUri = this.previews
							.values()
							.next().value?.sourceUri;
					}
				}
			},
			null,
	...
	private async updateEntry(
		...
		const generation = ++entry.updateGeneration;
		const resolvedDocument =
			document ?? (await vscode.workspace.openTextDocument(entry.sourceUri));
		if (!isPreviewableMarkdown(resolvedDocument)) {
			return;
		}

		entry.sourceUri = resolvedDocument.uri;
		const isDirty = resolvedDocument.isDirty;
		const diskText = await readDiskMarkdown(isDirty, () =>
			vscode.workspace.fs.readFile(resolvedDocument.uri),
		);
		const markdown = chooseMarkdownSource(
			isDirty,
			resolvedDocument.getText(),
			diskText,
		);
		...
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(
			resolvedDocument.uri,
		);
		if (workspaceFolder) {
			localResourceRoots.push(workspaceFolder.uri);
		}
		localResourceRoots.push(
			...collectLocalImageRootUris(markdown, baseUri.toString()).map((uri) =>
		...
		const rendered = renderMarkdown(markdown, toc, baseUri.toString(), (uri) =>
			entry.panel.webview.asWebviewUri(vscode.Uri.parse(uri)).toString(),
		);
		const config = getWorkbenchConfig();
		const checks = await runPreviewChecks(markdown, resolvedDocument.uri);

		if (generation !== entry.updateGeneration) {
			return;
		}
		...
		const state: PreviewState = {
			sourceUri: resolvedDocument.uri.toString(),
			sourceColumn: entry.sourceViewColumn,
	...
	private updateDiagnostics(
		...
		const diagnostics = checks.map((check) => {
			const endLineIndex = Math.min(
				check.endLine ?? check.line,
				Math.max(0, document.lineCount - 1),
			);
			const endChar =
				endLineIndex >= 0 ? document.lineAt(endLineIndex).text.length : 0;
	...
	private async handleMessage(
		...
		switch (message.type) {
			case "webviewReady": {
				...
				try {
					await this.updateEntry(entry);
					if (entry.initialFragment) {
						await entry.panel.webview.postMessage({
							type: "scrollToAnchor",
							value: entry.initialFragment,
						});
					}
				} catch (error) {
					console.error(
						"[markdown-lint] preview initial render failed:",
						error,
					);
					const replacement = entry.replaceAfterReady;
					if (replacement) {
						this.previews.set(getPreviewKey(entry.sourceUri), replacement);
						this.activeSourceUri = entry.sourceUri;
						entry.panel.dispose();
					}
					return;
				...
				const replacement = entry.replaceAfterReady;
				entry.replaceAfterReady = undefined;
				entry.initialFragment = undefined;
				if (replacement && this.getEntry(entry.sourceUri) === entry) {
					replacement.panel.dispose();
				}
				return;
			}
			case "setThemeMode":
			...
			case "setPreviewMode":
				...
				entry.previewMode = message.value;
				await this.updateAll();
				return;
			...
			case "revealLine": {
				...
				if (!editor) {
					editor = await this.openSourceEditorForEntry(entry);
				}
				entry.scrollSyncSuppressor.suppress("preview", 450);
				const line = clampEditorLine(message.value, editor.document.lineCount);
			...
			case "exportHtml":
				await exportHtml(
					await vscode.workspace.openTextDocument(entry.sourceUri),
					this.context,
				);
				return;
			case "openLink":
	...
	private async openLink(entry: PreviewEntry, href: string): Promise<void> {
		...
		if (target.type === "external") {
			if (!isAllowedExternalHref(target.href)) {
				void vscode.window.showWarningMessage(
					"Blocked unsupported link scheme.",
				);
				return;
			}
			...
			if (isPreviewableMarkdown(doc)) {
				if (entry.surface === "custom") {
					await vscode.commands.executeCommand(
						"vscode.openWith",
						targetUri.with({ fragment: target.fragment }),
	...
	private async openNonMarkdownDocument(
		document: vscode.TextDocument,
	): Promise<void> {
		await vscode.window.showTextDocument(document, {
			preview: false,
	...
	private getHtml(webview: vscode.Webview): string {
		const activeHeadingTrackerUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"activeHeadingTracker.js",
			),
		);
		const codeBlockControlsUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"codeBlockControls.js",
			),
		);
		const imageLightboxUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"imageLightbox.js",
		...
		const mermaidEnhancementUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"mermaidEnhancement.js",
			),
		);
		const mermaidInteractionUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"mermaidInteraction.js",
			),
		);
		const mermaidRuntimeUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"mermaidRuntime.js",
		...
		const scrollSyncRuntimeUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"scrollSyncRuntime.js",
			),
		);
		const tableEnhancementUri = webview.asWebviewUri(
			vscode.Uri.joinPath(
				this.context.extensionUri,
				"media",
				"tableEnhancement.js",
...
<html lang="en">
  ...
  <body>
      ...
      <div class="floating-menu" id="floating-menu">
        ...
        <button class="floating-menu-group" data-group="placement" type="button" aria-expanded="false">
          ...
          <span class="floating-menu-group-copy">
           ...
           <span class="floating-menu-group-value" id="placement-value">Inline</span>
          <span class="floating-menu-group-arrow">&#9656;</span>
        </button>
    ...
    <script nonce="${nonce}" src="${tableEnhancementUri}"></script>
...
function getNonce(): string {
	// 优先使用 crypto.randomUUID（Node.js 14+）
...
function resolveBesideColumn(
	...
	if (
		...
		return (sourceColumn + 1) as vscode.ViewColumn;
	}

	return vscode.ViewColumn.Beside;
}

function clampEditorLine(line: number, lineCount: number): number {
	return Math.max(0, Math.min(line, Math.max(0, lineCount - 1)));
}

function isSerializedPreviewState(value: unknown): value is {
	sourceUri: string;
	sourceColumn?: number;
	scrollLine?: number;
} {
	...
	return (
		typeof state.sourceUri === "string" &&
		(state.sourceColumn === undefined ||
			typeof state.sourceColumn === "number") &&
		(state.scrollLine === undefined || typeof state.scrollLine === "number")
	);
}
