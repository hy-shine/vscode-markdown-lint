...
test("flags local image references as missing-image candidates", () => {
	...
	const checks = findMissingImages(
		md.join("\n"),
		fenced(md.join("\n")),
		"file:///tmp/docs/",
	);
...
test("preserves source lines when combining checks after front matter", () => {
	const md = [
		"---",
		"title: Test",
		"---",
	...
	const checks = findAllChecks(md, "file:///tmp/docs/");

	assert.deepEqual(
		checks.map((check) => ({ type: check.type, line: check.line })),
		[
			{ type: "heading-skip", line: 4 },
			{ type: "missing-alt", line: 5 },
			{ type: "missing-image", line: 5 },
			{ type: "broken-link", line: 6 },
		],
	);
});
...
test("filters target checks with concurrent stat lookups while preserving order", async () => {
	const checks: PreviewCheck[] = [
		{ line: 0, type: "missing-alt", message: "Missing alt text" },
		{
			...
			targetUri: "file:///tmp/docs/existing.png",
		},
		{
			line: 2,
			type: "broken-link",
			message: "Link missing",
			targetUri: "file:///tmp/docs/missing.md",
		},
		{
			line: 3,
			type: "missing-image",
			message: "Image missing",
			targetUri: "file:///tmp/docs/slow-missing.png",
		},
	];
	const calls: string[] = [];
	let activeStats = 0;
	...
	const result = await filterExistingTargetChecks(checks, async (targetUri) => {
		...
		await new Promise((resolve) =>
			setTimeout(resolve, targetUri.includes("existing") ? 30 : 10),
		);
		activeStats -= 1;
