...
import { resolvePreviewLinkTarget } from '../src/core/links';

test('classifies external links for browser opening', () => {
  assert.deepEqual(resolvePreviewLinkTarget('file:///Users/jessy/docs/', 'https://example.com/a b'), {
    type: 'external',
    href: 'https://example.com/a b',
  });
  assert.deepEqual(resolvePreviewLinkTarget('file:///Users/jessy/docs/', 'mailto:author@example.com'), {
    type: 'external',
...
test('classifies same-document anchors without local file access', () => {
  assert.deepEqual(resolvePreviewLinkTarget('file:///Users/jessy/docs/', '#intro'), {
    type: 'anchor',
    fragment: 'intro',
  });
});
...
test('classifies relative Markdown links with normalized local file URI and fragment', () => {
  assert.deepEqual(resolvePreviewLinkTarget(
