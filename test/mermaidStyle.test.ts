...
test('defines style-specific syntax colors for non-paper presets', () => {
  for (const style of ['github', 'notion', 'tokyo-night', 'obsidian', 'typora']) {
    assert.match(css, new RegExp(`body\\.style-${style}[\\s\\S]*--syntax-text:`));
  }

  assert.match(css, /:is\(\.hljs, \.language-bash\)[\s\S]*color: var\(--syntax-text\)/);
  assert.match(css, /:is\(\.hljs-keyword, \.hljs-doctag, \.hljs-formula\)[\s\S]*color: var\(--syntax-keyword\)/);
  assert.match(css, /:is\(\.hljs-string, \.hljs-regexp, \.hljs-addition, \.hljs-attribute, \.hljs-meta \.hljs-string\)[\s\S]*color: var\(--syntax-string\)/);
});
...
test('keeps Notion dark and Obsidian notes close to their references', () => {
  const obsidianQuote = cssRule('body.style-obsidian .preview-content blockquote');
